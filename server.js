...
async function filterFilesForUser(files, username, role) {
  ...
  const restrictedFiles = new Set(rows.map(r => r.file_name));
  const allowedForUser = new Set(rows.filter(r => r.username === username).map(r => r.file_name));
  return files.filter(f => !restrictedFiles.has(f.name) || allowedForUser.has(f.name));
}
