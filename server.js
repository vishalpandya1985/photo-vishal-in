...
async function initDb() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS file_permissions (
      ...
      UNIQUE KEY unique_file_user (file_name, username)
    )
  `);
...
// A file with NO rows in file_permissions is visible to everyone (open by
// default). Once at least one row exists for a file, only admins and the
// listed usernames can see/download it.
async function canUserSeeFile(fileName, username, role) {
  if (role === 'admin') return true;
  const [rows] = await pool.query('SELECT username FROM file_permissions WHERE file_name = ?', [fileName]);
  if (rows.length === 0) return true; // unrestricted
  return rows.some(r => r.username === username);
}

async function filterFilesForUser(files, username, role) {
  if (role === 'admin') return files;
  const [rows] = await pool.query('SELECT file_name, username FROM file_permissions');
  const byFile = {};
  rows.forEach(r => {
    if (!byFile[r.file_name]) byFile[r.file_name] = [];
    byFile[r.file_name].push(r.username);
  });
  return files
    .filter(f => {
      const allowed = byFile[f.name];
      return !allowed || allowed.includes(username);
    })
    .map(f => {
      const allowed = byFile[f.name];
      let visibility = 'public';
      if (allowed) {
        visibility = (allowed.length === 1 && allowed[0] === username) ? 'onlyMe' : 'custom';
      }
      return { ...f, visibility };
    });
}
...
app.post('/login', async (req, res) => {
  ...
  if (user && bcrypt.compareSync(password, user.password_hash)) {
    ...
    return res.redirect('/?justLoggedIn=1');
  }
...
// Get who is currently allowed to see a file (empty list = visible to everyone)
app.get('/admin/files/:name/visibility', requireAdmin, async (req, res) => {
  const safeName = path.basename(req.params.name);
  const [rows] = await pool.query('SELECT username FROM file_permissions WHERE file_name = ?', [safeName]);
  res.json({ file: safeName, allowedUsers: rows.map(r => r.username) });
});

// Set exactly who can see a file. Send an empty array to make it visible to everyone again.
app.post('/admin/files/:name/visibility', requireAdmin, async (req, res) => {
  const safeName = path.basename(req.params.name);
  const { allowedUsers } = req.body;
  if (!Array.isArray(allowedUsers)) return res.status(400).json({ error: 'allowedUsers must be an array.' });
  await pool.query('DELETE FROM file_permissions WHERE file_name = ?', [safeName]);
  for (const username of allowedUsers) {
    await pool.query(
      'INSERT IGNORE INTO file_permissions (file_name, username) VALUES (?, ?)',
      [safeName, username]
    );
  }
  res.json({ file: safeName, allowedUsers });
});
...
app.get('/files', async (req, res) => {
  fs.readdir(UPLOAD_DIR, async (err, filenames) => {
    ...
    files = await filterFilesForUser(files, req.session.username, req.session.role);
    res.json({ files });
  });
});

app.get('/trash', async (req, res) => {
  fs.readdir(TRASH_DIR, async (err, filenames) => {
...
// Any logged-in user can mark a file "only me" or make it public again.
// If an admin has shared the file with specific other people, this is
// blocked for non-admins so it can't be undone by accident.
app.post('/files/:name/self-visibility', async (req, res) => {
  const safeName = path.basename(req.params.name);
  const { onlyMe } = req.body;
  const username = req.session.username;
  const role = req.session.role;

  const [rows] = await pool.query('SELECT username FROM file_permissions WHERE file_name = ?', [safeName]);
  const allowed = rows.map(r => r.username);
  const isPublic = allowed.length === 0;
  const isOnlyMe = allowed.length === 1 && allowed[0] === username;

  if (role !== 'admin' && !isPublic && !isOnlyMe) {
    return res.status(403).json({ error: 'An admin has shared this file with specific people. Ask an admin to change it.' });
  }

  await pool.query('DELETE FROM file_permissions WHERE file_name = ?', [safeName]);
  if (onlyMe) {
    await pool.query('INSERT IGNORE INTO file_permissions (file_name, username) VALUES (?, ?)', [safeName, username]);
  }
  res.json({ file: safeName, visibility: onlyMe ? 'onlyMe' : 'public' });
});

app.get('/files/:name', async (req, res) => {
  const safeName = path.basename(req.params.name);
  const allowed = await canUserSeeFile(safeName, req.session.username, req.session.role);
  if (!allowed) return res.status(403).json({ error: 'You do not have permission to view this file.' });
...
app.post('/download-zip', async (req, res) => {
  ...
  for (const name of names) {
    ...
    if (!allowed) continue;
    const filePath = path.join(UPLOAD_DIR, safeName);
    if (fs.existsSync(filePath)) archive.file(filePath, { name: safeName });
  }
  archive.finalize();
});
